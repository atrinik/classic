...
static const char module_doc_type[] = "Contains all the Atrinik object types as constants.";

/** The Atrinik Gender module docstring. */
static const char module_doc_gender[] = "Contains gender related constants.";
...
static const Atrinik_Constant constants[] = {
    ...
    {"CLIENT_CMD_INTERFACE", CLIENT_CMD_INTERFACE},
    {"CLIENT_CMD_NOTIFICATION", CLIENT_CMD_NOTIFICATION},
    {"CLIENT_CMD_MAPSTATS", CLIENT_CMD_MAPSTATS},

    {"BANK_SYNTAX_ERROR", BANK_SYNTAX_ERROR},
    {"BANK_SUCCESS", BANK_SUCCESS},
    {"BANK_JOURNAL_ERROR", BANK_JOURNAL_ERROR},
    {"BANK_JOURNAL_AMBIGUOUS", BANK_JOURNAL_AMBIGUOUS},
 ...
 */
static const Atrinik_Constant constants_types[] = {{"PLAYER", PLAYER},
                                                   {"BULLET", BULLET},
                                                   {"ROD", ROD},
                                                   {"TREASURE", TREASURE},
                                                   {"POTION", POTION},
 ...
 * Gender constants.
 */
static const Atrinik_Constant constants_gender[] = {{"NEUTER", GENDER_NEUTER},
                                                    {"MALE", GENDER_MALE},
 ...
 * Color constants
 */
static const char *const constants_colors[][2] = {{"COLOR_WHITE", COLOR_WHITE},
...
static char *read_script_file(const char *filename) {
    ...
    if (source == NULL || fread(source, 1, (size_t)size, fp) != (size_t)size) {
        free(source);
        fclose(fp);
        return NULL;
    }
...
static PyCodeObject *compilePython(char *filename) {
    ...
    if (!cache || cache->cached_time < stat_buf.st_mtime) {
        ...
        code = (PyCodeObject *)Py_CompileString(source, filename, Py_file_input);
        free(source);
...
static int do_script(PythonContext *context, const char *filename, bool *execution_failed) {
    ...
    if (pycode != NULL) {
        ...
        PyDict_SetItemString(
            dict,
            "pl",
            wrap_player(context->activator != NULL && context->activator->type == PLAYER
                            ? CONTR(context->activator)
                            : NULL));
...
static const char doc_Atrinik_FindParty[] = ".. function:: FindParty(name).\n\n"
...
static PyObject *Atrinik_Logger(PyObject *self, PyObject *args) {
    ...
    logger_print(hooks->logger_get_level(mode), __FUNCTION__, __LINE__, string);
...
static const char doc_Atrinik_GetTicks[] = ".. function:: GetTicks().\n\n"
...
static PyObject *Atrinik_print(PyObject *self, PyObject *args) {
    ...
    for (Py_ssize_t i = 0; i < PyTuple_Size(args); i++) {
        ...
        hooks->stringbuffer_append_string(
            sb,
...
static PyObject *Atrinik_Eval(PyObject *self, PyObject *args) {
    ...
    code = (PyCodeObject *)Py_CompileString(s, "eval'd code", Py_file_input);
...
static PyObject *Atrinik_GetSettings(PyObject *self, PyObject *ignored) {
    ...
    PyDict_SetItemString(dict, "world_maker", Py_BuildBoolean(hooks->settings->world_maker));
    PyDict_SetItemString(dict, "unit_tests", Py_BuildBoolean(hooks->settings->unit_tests));
    PyDict_SetItemString(dict,
                         "plugin_unit_tests",
                         Py_BuildBoolean(hooks->settings->plugin_unit_tests));
    PyDict_SetItemString(dict,
                         "plugin_unit_test",
                         Py_BuildValue("s", hooks->settings->plugin_unit_test));
    PyDict_SetItemString(dict,
                         "magic_devices_level",
                         Py_BuildValue("b", hooks->settings->magic_devices_level));
    PyDict_SetItemString(dict,
                         "magic_devices_level",
                         Py_BuildValue("b", hooks->settings->magic_devices_level));
    PyDict_SetItemString(dict,
                         "item_power_factor",
                         Py_BuildValue("d", hooks->settings->item_power_factor));
    PyDict_SetItemString(dict,
                         "python_reload_modules",
                         Py_BuildBoolean(hooks->settings->python_reload_modules));
    PyDict_SetItemString(dict,
                         "default_permission_groups",
                         Py_BuildValue("s", hooks->settings->default_permission_groups));
    ...
    for (Py_ssize_t i = 0; i < ALLOWED_CHARS_NUM; i++) {
        PyList_SetItem(list_allowed_chars,
                       i,
                       Py_BuildValue("s", hooks->settings->allowed_chars[i]));
        PyList_SetItem(list_limits,
                       i,
                       Py_BuildValue("KK",
    ...
    PyDict_SetItemString(dict,
                         "recycle_tmp_maps",
                         Py_BuildBoolean(hooks->settings->recycle_tmp_maps));
    PyDict_SetItemString(dict, "http_url", Py_BuildValue("s", hooks->settings->http_url));
    return dict;
}
...
#if PY_MAJOR_VERSION >= 3 && PY_MINOR_VERSION >= 2
                ret = PyEval_EvalCode((PyObject *)eval->code, eval->globals, eval->locals);
#else
...
static PyModuleDef AtrinikModule =
    {PyModuleDef_HEAD_INIT, "Atrinik", NULL, -1, AtrinikMethods, NULL, NULL, NULL, NULL};
    ...
    switch (field->type) {
        ...
        case FIELDTYPE_CSTR:
            if (value == Py_None || PyString_Check(value)) {
                if (*(char **)field_ptr != NULL) {
                    xfree(*(char **)field_ptr);
                }

                if (value == Py_None) {
                    *(char **)field_ptr = NULL;
                } else {
                    *(char **)field_ptr = xstrdup(PyString_AsString(value));
                }
            } else {
        ...
        case FIELDTYPE_INT8:
            if (PyInt_Check(value)) {
                ...
                if (PyErr_Occurred() || val < INT8_MIN || val > INT8_MAX) {
                    PyErr_SetString(PyExc_OverflowError, "Invalid integer value for int8 field.");
                    return -1;
                }
        ...
        case FIELDTYPE_INT16:
            if (PyInt_Check(value)) {
                ...
                if (PyErr_Occurred() || val < INT16_MIN || val > INT16_MAX) {
                    PyErr_SetString(PyExc_OverflowError, "Invalid integer value for int16 field.");
                    return -1;
                }
        ...
        case FIELDTYPE_INT32:
            if (PyInt_Check(value)) {
                ...
                if (PyErr_Occurred() || overflow != 0 || val < INT32_MIN || val > INT32_MAX) {
                    PyErr_SetString(PyExc_OverflowError, "Invalid integer value for int32 field.");
                    return -1;
                }
        ...
        case FIELDTYPE_INT64:
            if (PyInt_Check(value)) {
                ...
                if (PyErr_Occurred()) {
                    PyErr_SetString(PyExc_OverflowError, "Invalid integer value for int64 field.");
                    return -1;
                }
        ...
        case FIELDTYPE_PACKET: {
            ...
            if (len > packet->size) {
                PyErr_Format(PyExc_OverflowError, "Data too large for the packet to hold.");
                return -1;
            }
...
PyObject *generic_field_getter(fields_struct *field, void *ptr) {
    ...
    switch (field->type) {
        ...
        case FIELDTYPE_PACKET: {
            packet_struct *packet = *(packet_struct **)field_ptr;
            return PyBytes_FromStringAndSize((const char *)packet->data, packet->len);
        }
