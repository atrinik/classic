...
static fields_struct fields[] = {
    ...
    {"previous",
     FIELDTYPE_MAP,
     offsetof(mapstruct, prev),
     FIELDFLAG_READONLY,
     0,
     "Previous map in a doubly-linked list.; Atrinik.Map.Map or None "
     "(readonly)"},

    {"name", FIELDTYPE_SHSTR, offsetof(mapstruct, name), 0, 0, "Name of the map.; str or None"},
    {"msg",
     FIELDTYPE_CSTR,
     offsetof(mapstruct, msg),
     0,
     0,
     "Message map creator may have left.; str or None"},
    {"reset_timeout",
     FIELDTYPE_UINT32,
     offsetof(mapstruct, reset_timeout),
     0,
     0,
    ...
    {"timeout",
     FIELDTYPE_INT32,
     offsetof(mapstruct, timeout),
     0,
     0,
     "When this reaches 0, the map will be swapped out.; int"},
    {"difficulty",
     FIELDTYPE_UINT16,
     offsetof(mapstruct, difficulty),
     0,
     0,
    ...
    {"height",
     FIELDTYPE_UINT16,
     offsetof(mapstruct, height),
     FIELDFLAG_READONLY,
     0,
     "Height of the map.; int"},
    {"width",
     FIELDTYPE_UINT16,
     offsetof(mapstruct, width),
     FIELDFLAG_READONLY,
     0,
     "Width of the map.; int"},
    {"darkness",
     FIELDTYPE_UINT8,
     offsetof(mapstruct, darkness),
     0,
     0,
    ...
    {"path",
     FIELDTYPE_SHSTR,
     offsetof(mapstruct, path),
     FIELDFLAG_READONLY,
     0,
     "Path to the map file.; str (readonly)"},
    {"enter_x",
     FIELDTYPE_UINT8,
     offsetof(mapstruct, enter_x),
     0,
     0,
    ...
    {"region",
     FIELDTYPE_REGION,
     offsetof(mapstruct, region),
     FIELDFLAG_READONLY,
     0,
     "Region the map is in.; "
     "Atrinik.Region.Region or None (readonly)"},
    {"bg_music",
     FIELDTYPE_SHSTR,
     offsetof(mapstruct, bg_music),
     0,
     0,
     "Background music of the map.; str or None"},
    {"weather",
     FIELDTYPE_SHSTR,
     offsetof(mapstruct, weather),
     0,
     0,
 ...
 */
static char *mapflag_names[] = {"f_outdoor",
                                "f_unique",
                                "f_fixed_rtime",
                                "f_nomagic",
                                "f_height_diff",
...
static PyObject *Atrinik_Map_DrawInfo(Atrinik_Map *self, PyObject *args, PyObject *keywds) {
    ...
    hooks->draw_info_map(type, name, color, self->map, x, y, distance, NULL, NULL, message);
...
static PyObject *Atrinik_Map_Blocked(Atrinik_Map *self, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(args, "O!iii", &Atrinik_ObjectType, &obj, &x, &y, &terrain)) {
        return NULL;
    }
...
static PyObject *Atrinik_Map_FreeSpot(Atrinik_Map *self, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(args, "O!iiii", &Atrinik_ObjectType, &obj, &x, &y, &start, &stop)) {
        return NULL;
    }
    ...
    if (stop >= SIZEOFFREE) {
        PyErr_SetString(PyExc_ValueError, "'stop' cannot be higher than or equal to SIZEOFFREE.");
        return NULL;
    }
    ...
    return Py_BuildValue("i", hooks->map_free_spot(m, x, y, start, stop, obj->obj->arch, obj->obj));
}
...
static const char doc_Atrinik_Map_Save[] = ".. method:: Save().\n\n"
...
static PyObject *Atrinik_Map_RichCompare(Atrinik_Map *left, Atrinik_Map *right, int op) {
    if (left == NULL || right == NULL || !PyObject_TypeCheck((PyObject *)left, &Atrinik_MapType) ||
...
PyTypeObject Atrinik_MapType = {
    PyVarObject_HEAD_INIT(NULL, 0).tp_name = "Atrinik.Map",
