...
- Mark only serialized, visible `EXIT` objects with an established usable
  ...
  same-map coordinate, tiled-direction, and paired auto-linked subtype exits
  are eligible. Destination checks for rendering neither load maps nor choose
  randomly among auto-linked exits. The client outlines eligible objects after
  the complete world pass only at the player's physical depth. This
  presentation does not broaden line of sight or disclose layer-0/system
  exits, unexplored transitions, or hidden objects that the server did not
  serialize.
