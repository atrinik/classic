...
#define LOGIN_TEXT_INPUT_MAX \
    ((button_tab_login.pressed_forced ? LOGIN_TEXT_INPUT_PASSWORD : LOGIN_TEXT_INPUT_PASSWORD2) + 1)
...
static int text_input_character_check(text_input_struct *text_input, char c) {
    if (text_input == &text_inputs[LOGIN_TEXT_INPUT_NAME] &&
        !char_contains(c, s_settings->text[SERVER_TEXT_ALLOWED_CHARS_ACCOUNT])) {
        return 0;
    } else if ((text_input == &text_inputs[LOGIN_TEXT_INPUT_PASSWORD] ||
                text_input == &text_inputs[LOGIN_TEXT_INPUT_PASSWORD2]) &&
               !char_contains(c, s_settings->text[SERVER_TEXT_ALLOWED_CHARS_PASSWORD])) {
        return 0;
    }
...
static int popup_draw(popup_struct *popup) {
    ...
    text_show(popup->surface,
              FONT_ARIAL12,
              "Account name &lsqb;[tooltip=Enter your account's name.][h=#" COLOR_HGOLD
              "]?[/h][/tooltip]&rsqb;",
              50,
              92,
              COLOR_WHITE,
    ...
    text_show(popup->surface,
              FONT_ARIAL12,
              "Password &lsqb;[tooltip=Enter your password.][h=#" COLOR_HGOLD
              "]?[/h][/tooltip]&rsqb;",
              50,
              132,
              COLOR_WHITE,
    ...
    if (button_tab_register.pressed_forced) {
        text_show(popup->surface,
                  FONT_ARIAL12,
                  "Verify password &lsqb;[tooltip=Enter your password again.][h=#" COLOR_HGOLD
                  "]?[/h][/tooltip]&rsqb;",
                  50,
                  172,
                  COLOR_WHITE,
    ...
    button_done.x = text_inputs[LOGIN_TEXT_INPUT_NAME].coords.x +
                    text_inputs[LOGIN_TEXT_INPUT_NAME].coords.w -
...
void login_start(void) {
    ...
    button_tab_login.pressed_forced = 1;
    button_tab_login.texture = button_tab_register.texture =
        texture_get(TEXTURE_TYPE_CLIENT, "button_tab");
    button_tab_login.texture_over = button_tab_register.texture_over =
        texture_get(TEXTURE_TYPE_CLIENT, "button_tab_over");
    button_tab_login.texture_pressed = button_tab_register.texture_pressed =
        texture_get(TEXTURE_TYPE_CLIENT, "button_tab_down");
    ...
    text_inputs[LOGIN_TEXT_INPUT_PASSWORD].show_edit_func =
        text_inputs[LOGIN_TEXT_INPUT_PASSWORD2].show_edit_func = text_input_show_edit_password;
    text_input_current = LOGIN_TEXT_INPUT_NAME;
