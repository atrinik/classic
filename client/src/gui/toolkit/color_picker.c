 ...
 * otherwise.
 */
static int color_picker_element_show(SDL_Surface *surface,
    ...
    if (type == COLOR_PICKER_ELEM_COLOR) {
        ...
        for (x = 0; x < box.w; x++) {
            ...
            if (color_picker->hsv[2] >= hsv[2] &&
                color_picker->hsv[2] < hsv[2] + (1.0 * (1.0 / (double)box.w))) {
                selx = x;
            }
            ...
            for (y = 0; y < box.h; y++) {
                ...
                if (color_picker->hsv[1] >= hsv[1] &&
                    color_picker->hsv[1] < hsv[1] + (1.0 * (1.0 / (double)box.h))) {
                    sely = y;
                }
                ...
                if (event) {
                    if (mx >= dest.x && my >= dest.y && mx < dest.x + dest.w &&
                        my < dest.y + dest.h) {
        ...
        for (y = 0; y < box.h; y++) {
            ...
            if (color_picker->hsv[0] >= hsv[0] &&
                color_picker->hsv[0] < hsv[0] + (1.0 * (1.0 / (double)box.h))) {
                r = 255 - r;
                g = 255 - g;
                b = 255 - b;
            }
...
int color_picker_mouse_over(color_picker_struct *color_picker, int mx, int my) {
    ...
    for (i = 0; i < COLOR_PICKER_ELEM_NUM; i++) {
        ...
        if (mx >= x && my >= y && mx < x + color_picker->elements[i].coords.w &&
            my < y + color_picker->elements[i].coords.h) {
            return 1;
        }
    }
