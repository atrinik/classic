...
static void widget_background(widgetdata *widget, int draw) {
    ...
    if (MapData.name_fadeout_start || MapData.name[0] == '\0') {
        ...
        if (time_passed > MAP_NAME_FADEOUT || MapData.name[0] == '\0') {
            if (MapData.name[0] != '\0') {
                alpha =
                    MIN(255, 255 * ((double)(time_passed - MAP_NAME_FADEOUT) / MAP_NAME_FADEOUT));
            ...
            if (MapData.name_new[0] != '\0') {
                ...
                resize_widget(widget,
                              RESIZE_RIGHT,
                              text_get_width(MAP_NAME_FONT, MapData.name, TEXT_MARKUP));
                resize_widget(widget,
                              RESIZE_BOTTOM,
                              text_get_height(MAP_NAME_FONT, MapData.name, TEXT_MARKUP));
