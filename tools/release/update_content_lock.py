...
def release_by_tag(
    releases: list[dict[str, object]], tag: str
...
def verify_current_coordinate(
    ...
    if release is None:
        # A missing current release is the authenticated one-time history
        # cutover signal. The lock syntax and canonical URL were checked above,
        # while every replacement candidate still passes complete verification.
        return semver, True
...
def execute(
    ...
    with tempfile.TemporaryDirectory(prefix="atrinik-content-update-") as directory_name:
        ...
        current_version, current_is_historical = verify_current_coordinate(
            current, releases, api, directory, downloader
        )
        lineage_mode = "cutover" if current_is_historical else "strict-descendant"
        ...
        for release in releases:
            ...
            try:
                ...
                continue
            if not current_is_historical and version <= current_version:
                continue
            try:
...
def write_pr_body(evidence: dict[str, object], path: Path) -> None:
    ...
    lineage_line = (
        "- Applied the one-time history cutover for the authenticated historical "
        "coordinate; the replacement still passed every tag, asset, checksum, "
        "archive, manifest, and compatibility check."
        if evidence.get("lineage_mode") == "cutover"
        else "- Required the candidate commit to be a strict descendant of the "
        "current verified content coordinate."
    )
    lines.extend([
        "",
        "## Verification",
        ...
        "- Required the Classic target to originate from `content@main`.",
        lineage_line,
